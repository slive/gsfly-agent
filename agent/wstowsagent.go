...
func NewWsToWs(serverConf *strap.HttpxServerConf, clientConfs ...*strap.WsClientConf) Agent {
...
func (ka *WsToWsAgent) Start() error {
	id := ka.GetId()
	if !ka.IsClosed() {
	...
	conf := ka.GetServerConf().(*strap.HttpxServerConf)
	server := strap.NewHttpxServer(conf)
	wsServer := server.(*strap.HttpWsServer)
	// TODO 如何配置path
	wsServer.AddWsHandleFunc("ctl/ws", chHandle)
	err := server.Start()
	if err == nil {
		ka.Server = server
	}
...
func (ka *WsToWsAgent) srcHandleFunc(packet gch.Packet) error {
	defer func() {
		err := recover()
		if err != nil {
...
func (ka *WsToWsAgent) dstHandleFunc(packet gch.Packet) error {
